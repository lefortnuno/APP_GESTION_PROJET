...
Individu.deleteIndividu = (cin, result) => {
  dbConn.query(`DELETE FROM Individu WHERE cin = ${cin}`, function (err, res) {
    if (err) {
      result(err, null);
    } else {
      result(null, { success: true });
