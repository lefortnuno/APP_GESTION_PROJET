...
import { utilisateurs } from "../../../api/data_array.js";
...
export default function Utilisateur() {
  ...
  const showDeleteModal = () => { 
    setDeleteMessage(
      `Etes vous sûre de vouloir supprimer l'utilisateur "El Idrissi Ahmed N°UT001" definitivement de notre base de donnée ?`
    );
  ...
  const currentItems = utilisateurs.slice(indexOfFirstItem, indexOfLastItem);
                          ...
                          <tbody>
                            {contenuTab || users.length !== 0 ? (
                              currentItems.map((user, key) => (
                                <tr key={key}>
                                  <th scope="row">{user.MatriculeUtilisateur} </th> 
                                  <td>
                                    {user.NomUtilisateur} {user.PrenomUtilisateur}
                                  </td>
                                  <td>{user.TelUtilisateur}</td>
